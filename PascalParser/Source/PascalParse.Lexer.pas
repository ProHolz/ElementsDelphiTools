...
type
  ...
  protected
    ...
    method ExpectedEx(const Syms: TptTokenKinds); virtual;
    ...
    method NextTokenAssembly; virtual;
    ...
    method AnonymousMethodTypeFunction; virtual;
    method AnonymousMethodTypeProcedure; virtual;
    method ArrayConstant; virtual;
    method ArrayBounds; virtual;
    method ArrayDimension; virtual;
    method ArrayOfConst; virtual;
    method ArraySubType; virtual;
    method ArrayType; virtual;
    method AsmFragment; virtual;
    method AsmLabelAt; virtual;
    method AsmStatement; virtual;
    method AsmStatements; virtual;
    ...
    method CompilerDirective; virtual;
    ...
    method DirectiveSealed; virtual;
    method DirectiveStatic; virtual;
    method DispInterfaceForward; virtual;
    method DispIDSpecifier; virtual;
    method DoubleAddressOp; virtual;
    ...
    method ExternalDependency; virtual;
...
begin
  NextToken;
  if TokenID = TptTokenKind.ptSemiColon then NextToken;      //#166
end;
...
method TmwSimplePasPar.ExpectedEx(const Syms: TptTokenKinds);
var
Symbols: String;
&Optional: String;
begin
  &Optional := '';
  Symbols := '';
  if Syms.Contains(Lexer.ExID) then NextToken
  else if (Lexer.TokenID = TptTokenKind.ptNull) or assigned(OnMessage) then
  begin
    for Sym in Syms do begin
      Symbols:= Symbols + &Optional + Sym.EnumName;
      &Optional:= ' or ';
    end;
    if (Lexer.TokenID = TptTokenKind.ptNull) then
      raise new ESyntaxError(String.Format('{0} {1}', [Symbols, rsEndOfFile]), fLexer.PosXY)
    else if assigned(OnMessage) then begin
      OnMessage(Self, TMessageEventType.meError, String.Format('{0} {1}', ['EX:' + Symbols, fLexer.Token]),
      fLexer.PosXY.X, fLexer.PosXY.Y);
    end;
  end;
end;
...
method TmwSimplePasPar.NextTokenAssembly;
begin
...
begin
  case TokenID of
    TptTokenKind.ptConstructor, TptTokenKind.ptDestructor, TptTokenKind.ptProcedure, TptTokenKind.ptFunction:
    begin
      NextToken;
    end;
    else
...
begin
  ...
  InitAhead;
  fAheadParse.NextToken;
  fAheadParse.FunctionProcedureName;

  if fAheadParse.TokenID = TptTokenKind.ptEqual then
...
method TmwSimplePasPar.DirectiveAbstract;
begin
  ExpectedEx(TptTokenKind.ptAbstract);  //abstract is an ExID.
end;

method TmwSimplePasPar.DirectiveAssembler;
begin
  ExpectedEx(TptTokenKind.ptAssembler);
end;

method TmwSimplePasPar.DirectiveBinding;
begin
  case ExID of
    TptTokenKind.ptAbstract, TptTokenKind.ptVirtual, TptTokenKind.ptDynamic, TptTokenKind.ptMessage, TptTokenKind.ptOverride, TptTokenKind.ptOverload,
    TptTokenKind.ptReintroduce, TptTokenKind.ptFinal, TptTokenKind.ptStatic: begin
      NextToken;
    end
    else begin
...
begin
  ...
  if (fLexer.ExID = TptTokenKind.ptDependency) then ExternalDependency;
end;
...
method TmwSimplePasPar.ExternalDependency;
begin
  ExpectedEx(TptTokenKind.ptDependency);
  Identifier;
  while TokenID = TptTokenKind.ptComma do begin
    NextToken;
    Identifier;
  end; {while}
  Semicolon;
...
method TmwSimplePasPar.RecordVariantTag;
begin
  Identifier;
...
method TmwSimplePasPar.AsmStatements;
begin
  Lexer.AsmCode := True;
  Expected(TptTokenKind.ptAsm);
{ should be replaced with a Assembler lexer }
  while TokenID <> TptTokenKind.ptEnd do begin
    case fLexer.TokenID of
      ...
      TptTokenKind.ptCRLF: //empty line
      NextTokenAssembly;
      else begin
        AsmStatement;
...
begin
  while not(Lexer.TokenID in [TptTokenKind.ptCRLF]) do begin
    case TokenID of
      TptTokenKind.ptAddressOp, TptTokenKind.ptDoubleAddressOp: AsmLabelAt;
      else AsmFragment;
    end;
  end;
end;

method TmwSimplePasPar.AsmFragment;
begin
  NextTokenAssembly;
end;
method TmwSimplePasPar.AsmLabelAt;
begin
  NextTokenAssembly;
end;
...
begin
  ...
  begin
    InitAhead;
    fAheadParse.Designator;

    if fAheadParse.TokenID in [TptTokenKind.ptAssign, TptTokenKind.ptSemiColon, TptTokenKind.ptElse] then
      SimpleStatement
    else
      Expression;
  end;
...
begin
  ...
  begin
    if (ExID in [TptTokenKind.ptBoolean,TptTokenKind.ptByte,TptTokenKind.ptByteBool,TptTokenKind.ptCardinal,TptTokenKind.ptChar,TptTokenKind.ptAnsiChar,TptTokenKind.ptDWORD,
        TptTokenKind.ptInt64,TptTokenKind.ptUInt64,TptTokenKind.ptInteger,TptTokenKind.ptLongBool,TptTokenKind.ptLongint,TptTokenKind.ptLongword,
        TptTokenKind.ptPChar,TptTokenKind.ptShortint,
        TptTokenKind.ptSmallint,TptTokenKind.ptWideChar,TptTokenKind.ptWord,TptTokenKind.ptWordBool]) then
    begin
      NextToken;
    end else begin
...
begin
  case TokenID of
    ...
    begin
        InitAhead;
        fAheadParse.NextToken;
        fAheadParse.TypeArgs;

        if fAheadParse.TokenID = TptTokenKind.ptGreater then
        begin
          NextToken;
          TypeArgs;
...
begin
  if TokenID = TptTokenKind.ptSquareOpen then  //#229
    CustomAttribute;
  FieldNameList;
  Expected(TptTokenKind.ptColon);
  TypeKind;
...
begin
  ...
  begin
    ...
    while TheTokenID in [TptTokenKind.ptAbstract, TptTokenKind.ptCdecl, TptTokenKind.ptDynamic, TptTokenKind.ptExport, TptTokenKind.ptExternal, TptTokenKind.ptFar,
      TptTokenKind.ptMessage, TptTokenKind.ptNear, TptTokenKind.ptOverload, TptTokenKind.ptOverride, TptTokenKind.ptPascal, TptTokenKind.ptRegister,
      TptTokenKind.ptReintroduce, TptTokenKind.ptSafeCall, TptTokenKind.ptStdcall, TptTokenKind.ptVirtual, TptTokenKind.ptStatic, TptTokenKind.ptInline, TptTokenKind.ptVarargs] do
...
begin
  case TokenID of
    ...
    case fLexer.TokenID of
      ...
      begin
          InitAhead;
          fAheadParse.NextToken;
          fAheadParse.SimpleExpression;
          if fAheadParse.TokenID = TptTokenKind.ptDotDot then
            SubrangeType
          else
            OrdinalType;
      end;
  ...
  method TmwSimplePasPar.ArrayOfConst;
  begin
    Expected(TptTokenKind.ptConst);
  end;
  method TmwSimplePasPar.ArraySubType;
  begin
    case TokenID of
      TptTokenKind.ptConst: ArrayOfConst;
      else TypeKind;
    end;
  end;
  ...
  method TmwSimplePasPar.DoubleAddressOp;
  begin
    Expected(TptTokenKind.ptDoubleAddressOp);
  end;
  ...
  begin
    case GenID of
      TptTokenKind.ptAbstract, TptTokenKind.ptFinal:
      begin
        DirectiveBinding;
      end;
  ...
  //TODO: Add FINAL
    while ExID in [TptTokenKind.ptAbstract, TptTokenKind.ptCdecl, TptTokenKind.ptDynamic, TptTokenKind.ptExport, TptTokenKind.ptExternal, TptTokenKind.ptFar,
      ...
      TptTokenKind.ptStatic, TptTokenKind.ptInline, TptTokenKind.ptAssembler, TptTokenKind.ptForward, TptTokenKind.ptDelayed, TptTokenKind.ptFinal] do
    begin
      case ExID of
    ...
    begin
      ...
      begin
        ...
        begin
          InitAhead;
          fAheadParse.NextToken;
          if fAheadParse.TokenID = TptTokenKind.ptLower then
            fAheadParse.TypeParams;
    ...
    begin
      case GenID of
        TptTokenKind.ptBoolean, TptTokenKind.ptByte, TptTokenKind.ptChar, TptTokenKind.ptAnsiChar, TptTokenKind.ptDWORD, TptTokenKind.ptInt64, TptTokenKind.ptUInt64, TptTokenKind.ptInteger, TptTokenKind.ptCardinal, TptTokenKind.ptLongint,
        ...
        TptTokenKind.ptAnsiString, TptTokenKind.ptShortString, TptTokenKind.ptWideString, TptTokenKind.ptUnicodeString:
        begin
          StringIdentifier;
        end;
  ...
  method TmwSimplePasPar.Resident;
  begin
    ExpectedEx(TptTokenKind.ptResident);
  end;

  method TmwSimplePasPar.CompilerDirective;
  begin
    ExpectedEx(TptTokenKind.ptCompDirect);
  end;
...
//        end;
  ...
  method TmwSimplePasPar.NameSpecifier;
  begin
    ExpectedEx(TptTokenKind.ptName);
    SimpleExpression;
  end;
  method TmwSimplePasPar.ClassTypeEnd;
  begin
    case ExID of
      TptTokenKind.ptExperimental: DirectiveExperimental;
  ...
  begin
    ExpectedEx([TptTokenKind.ptInline, TptTokenKind.ptAssembler]);
  end;
  ...
  method TmwSimplePasPar.DirectiveExperimental;
  begin
    ExpectedEx(TptTokenKind.ptExperimental);
  end;
  method TmwSimplePasPar.DirectiveDelayed;
  begin
    ExpectedEx(TptTokenKind.ptDelayed);
  end;
  method TmwSimplePasPar.DirectiveSealed;
  begin
    Expected(TptTokenKind.ptSealed);
  end;
  method TmwSimplePasPar.DirectiveStatic;
  begin
    ExpectedEx(TptTokenKind.ptStatic);
  end;
  ...
  begin
    case TokenID of
      ...
      begin
        NextToken;
        if TokenID = TptTokenKind.ptRoundOpen then begin
          FormalParameterList;
        end;
        Expected(TptTokenKind.ptColon);
        ReturnType;
  ...
  method TmwSimplePasPar.AnonymousMethodTypeProcedure;
  begin
    Expected(TptTokenKind.ptProcedure);
  end;
  method TmwSimplePasPar.AnonymousMethodTypeFunction;
  begin
    Expected(TptTokenKind.ptFunction);
  end;
  ...
  begin
    if fAheadParse = nil then
      fAheadParse := new TmwSimplePasPar(Compiler);
    fAheadParse.Lexer.InitFrom(Lexer);
  end;
  ...
  begin
    case TokenID of
      TptTokenKind.ptIn, TptTokenKind.ptOut, TptTokenKind.ptConst, TptTokenKind.ptVar, TptTokenKind.ptUnsafe:
      NextToken;
      else
        begin
          Expected(TptTokenKind.ptIdentifier);
