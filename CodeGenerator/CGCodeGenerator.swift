public __abstract class CGCodeGenerator {
	...
	internal final func generateTypeDefinition(_ type: CGTypeDefinition) {
		...
		} else if let type = type as? CGSetTypeDefinition {
			generateSetType(type)
	...
	internal func generateSetType(_ type: CGSetTypeDefinition) {
		   // descendant must override
		assert(false, "generateSetType not implemented")
	}
	...
	internal final func generateTypeReference(_ type: CGTypeReference, ignoreNullability: Boolean) {
		...
		} else if let type = type as? CGRangeTypeReference {
			generateRangeTypeReference(type, ignoreNullability: ignoreNullability)
	...
	internal func generateRangeTypeReference(_ type: CGRangeTypeReference, ignoreNullability: Boolean = false) {
		assert(false, "generateSubRangeTypeReference not implemented")
	}
