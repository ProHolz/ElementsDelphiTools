...
public __abstract class CGPascalCodeGenerator : CGCodeGenerator {
	...
	*/
	...
	override func generateBlockType(_ type: CGBlockTypeDefinition) {
		assert(false, "generateBlockTypeExpression is not supported in base Pascal, only Oxygene")
	}
	...
	override func generateSetType(_ type: CGSetTypeDefinition) {
		generateIdentifier(type.Name)
		Append(" = ")
		pascalGenerateTypeVisibilityPrefix(type.Visibility)
		Append("Set of (")
	...
	*/
	...
	override func generateRangeTypeReference(_ type: CGRangeTypeReference, ignoreNullability: Boolean = false) {
		generateExpression(type.Start)
		Append("..")
		generateExpression(type.End)
	}
